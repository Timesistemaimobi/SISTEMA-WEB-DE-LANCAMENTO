...
def parse_flexible_float(value_str):
    ...
    if last_comma > last_dot: # Provável decimal BR (,)
        ...
        num_str = cleaned_text.replace(',', '.')
    else: # Só ponto ou nenhum separador
    ...
    try:
        ...
        # print(f"DEBUG parse FALHA: '{value_str}' -> '{cleaned_text}' -> '{num_str}'")
        return None
...
def processar_tabela_precos_web(input_filepath, block_etapa_mapping):
    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            workbook = writer.book; worksheet = writer.sheets['Tabela Formatada']
            print("  Aplicando estilos visuais e formato de moeda...")
            # ... (definições de estilos e formato de moeda - igual v11) ...
            header_bg_color="FFFFFF"; etapa_header_bg_color="FFFFFF"; bloco_header_bg_color="FFFFFF"
            title_fill=PatternFill(start_color=header_bg_color, end_color=header_bg_color, fill_type="solid"); etapa_fill=PatternFill(start_color=etapa_header_bg_color, end_color=etapa_header_bg_color, fill_type="solid"); bloco_fill=PatternFill(start_color=bloco_header_bg_color, end_color=bloco_header_bg_color, fill_type="solid"); data_header_fill=PatternFill(start_color=header_bg_color, end_color=header_bg_color, fill_type="solid")
            title_font=Font(name='Calibri', size=11, bold=True, color="000000"); etapa_font=Font(name='Calibri', size=11, bold=True, color="000000"); bloco_font=Font(name='Calibri', size=11, bold=True, color="000000"); data_header_font=Font(name='Calibri', size=11, bold=True, color="000000"); data_font=Font(name='Calibri', size=11, bold=False, color="000000")
            center_align=Alignment(horizontal='center', vertical='center', wrap_text=True); thin_border_side=Side(style='thin', color="000000"); thin_border=Border(left=thin_border_side, right=thin_border_side, top=thin_border_side, bottom=thin_border_side); top_bottom_border=Border(top=thin_border_side, bottom=thin_border_side)
            brl_currency_format = 'R$ #,##0.00'; text_format = '@'
            # ... (aplicação de estilos e formato de moeda em loop - igual v11) ...
            ...
            for etapa_nome, etapa_info in row_map['etapas'].items():
                etapa_header_r = etapa_info['header_row']; worksheet.merge_cells(start_row=etapa_header_r, start_column=1, end_row=etapa_header_r, end_column=num_cols); cell_etapa = worksheet.cell(row=etapa_header_r, column=1); cell_etapa.fill = etapa_fill; cell_etapa.font = etapa_font; cell_etapa.alignment = center_align; cell_etapa.border = top_bottom_border
                for bloco_val_orig, rows_info in etapa_info['blocks'].items():
                    ...
                    worksheet.merge_cells(start_row=bloco_header_r, start_column=1, end_row=bloco_header_r, end_column=num_cols); cell_bloco = worksheet.cell(row=bloco_header_r, column=1); cell_bloco.fill = bloco_fill; cell_bloco.font = bloco_font; cell_bloco.alignment = center_align; cell_bloco.border = top_bottom_border
                    for c_idx in range(1, num_cols + 1): cell = worksheet.cell(row=data_header_r, column=c_idx); cell.fill = data_header_fill; cell.font = data_header_font; cell.alignment = center_align; cell.border = thin_border
        ...
        output.seek(0)
        print("(Tabela Preços Formatador - v12 Keywords CSV) Processamento concluído.")
        return output

    # --- Blocos except (iguais v11) ---
